...
BOOST_AUTO_TEST_CASE (auxpow_pow)
{
  ...
  block.SetBaseVersion (2, params.nAuxpowChainId[ALGO_SHA256D]);
  mineBlock (block, true);
  BOOST_CHECK (CheckProofOfWork (block, params));

  block.SetChainId (params.nAuxpowChainId[ALGO_SHA256D] + 1);
  mineBlock (block, true);
  BOOST_CHECK (!CheckProofOfWork (block, params));
  ...
  block.SetChainId (params.nAuxpowChainId[ALGO_SHA256D]);
